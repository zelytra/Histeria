    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if (sender instanceof Player) {
            ...
            if (player.hasPotionEffect(PotionEffectType.NIGHT_VISION)) {
                ...
                player.sendMessage(Message.getPlayerPrefixe() + "§aNight Vision effect has been removed.");
                return true;
            }
            PotionEffect nv = new PotionEffect(PotionEffectType.NIGHT_VISION, 99999999, 0, false, false, false);
            player.addPotionEffect(nv);
            player.sendMessage(Message.getPlayerPrefixe() + "§aNight Vision effect has been set.");
            return true;
        }
        return false;
    }
